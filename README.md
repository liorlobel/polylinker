...
Vulkan for wgpu — and on `macos-latest` on Apple's OpenGL and Metal. The Linux
leg is written from the packages' documentation and has not yet been seen green;
the macOS leg's two launches were measured exiting 0 on the maintainer's Mac that
day, not yet on a hosted runner. That is a Linux software adapter and one Apple
GPU, not a Windows driver, and no Windows leg launches it. `PL_GUI_RENDERER=wgpu`
