...
> crates and 178,657 lines of Rust, 100,153 of it dependency-free (144 `.rs`
> files under `crates/` and `bins/`), with 2,083 `#[test]` functions and a
...
> two that face outwards: `bins/pl-gui` (eframe, rfd, egui-phosphor, and on macOS only the objc2 trio for its menu bar and Dock icon) and
...
> natively, and the archive and the installer as well — packed, verified,
> installed and uninstalled on every push and pull request, which is more than
> this paragraph claimed until 2026-08-15. **The one thing it was held to and
> failed, it now passes.** Every ARM64 binary in v0.11.0 imported
> `VCRUNTIME140.dll`, a DLL that is not part of Windows and whose
> redistributable needs the administrator rights this project's whole premise
> says the user does not have; `.cargo/config.toml` scoped `+crt-static` to the
> x86-64 triple and named no ARM64 one. v0.11.1 declares it for both, and two
...
| `prototype/dna-reader.html` | **Usable today.** The same Rust core compiled to wasm32 and inlined into one HTML file: opens `.dna`, GenBank and FASTA, draws maps, digests, exports GenBank/FASTA/SVG. **Its map is its own — neither `pl-draw` nor the TypeScript package.** The page carries a ~230-line renderer of its own, `renderMap` branching to `renderCircular` and `renderLinear`, and it has drawn a linear molecule as a horizontal track since 2026-07-26 (`d20056b`): driven in jsdom on a 420 bp linear record it emits **no `<circle>` at all** and one horizontal axis, `x1=56 x2=620 y1=y2=96`, captioned `linfrag.gb · 420 bp linear`. The same fixture made circular gives two `<circle>`s and arced feature paths instead, so both branches work. **Until 2026-08-14 this cell said the opposite** — that the map was "the TypeScript renderer in `packages/circular-map`, not `pl-draw`", and that it therefore "still draws a linear molecule as a ring with a gap in it". Both halves were false on the day they were written, 2026-08-07 (`14250f2`), twelve days after `renderLinear` landed, and this page has never loaded anything out of `packages/` at all — no `<script src>`, no `import`, no fetch. Understating the capability was the smaller half of that error. The larger half is what the sentence hid: **four things in this repository draw a plasmid map, and what checks them against each other is thinner than four renderers deserve.** They are [`crates/pl-draw`](crates/pl-draw) (Rust; every exported figure, ring and track, from `pl export` and from the app's Save), [`bins/pl-gui/src/map.rs`](bins/pl-gui/src/map.rs) (the desktop app's on-screen map, painted straight to egui, sharing `pl_draw::ring`'s circular layout), [`packages/circular-map`](packages/circular-map) (TypeScript, standalone, **circular only** — *that* is the renderer that still draws a linear molecule as a gapped ring, `LINEAR_GAP` at `src/render.ts:83`) and this page's own. Of the six pairs, two are checked and four are not: `crates/pl-draw/tests/agreement.rs` replays a fixture generated from the TypeScript, and the gate regenerates that fixture rather than merely reading it, but the check is helper-level — it never builds a molecule and never compares an arc, a radius or a label column, which [`crates/pl-draw/src/lib.rs`](crates/pl-draw/src/lib.rs) says of itself; and the app's on-screen map is held to the exported figure only at the quantities that have already gone wrong once, the label anchor and the arrowhead count. **This page's renderer is checked against nothing.** `prototype/check_page.js` asserts only that the map has at least eight paths and no `NaN` in its geometry, never which of the two branches ran. A step of [`tools/ci.ps1`](tools/ci.ps1) does invoke it — *"the browser prototype: template and builder agree, and the page is built and driven where it can be"* — but only where jsdom is installed, and jsdom is on no runner, so on every CI leg that clause is false and the harness does not run. No install, no network, no account — runs from a USB stick on a locked-down PC. Built by [`tools/build-web.ps1`](tools/build-web.ps1), which substitutes `{{WASM_BASE64}}`, `{{DEMO_GENBANK}}` and `{{BUILD_STAMP}}` and nothing else; not committed, because it is 257 KB of base64 that changes every rebuild. |
...
| Windows on ARM64 | **`polylinker-<version>-windows-arm64.zip` and `polylinker-<version>-windows-arm64.msi`** — the same two downloads x86-64 gets, from the same [`tools/release.ps1`](tools/release.ps1) and [`tools/build-msi.ps1`](tools/build-msi.ps1), with the same licence texts and the same `SHA256SUMS.txt` inside. Everything else in this row is what is **not** established about them, because this is the one platform in the table that has never been in a human being's hands. **The build happens on GitHub's free `windows-11-arm` runners and can happen nowhere else here.** `aarch64-pc-windows-msvc` is installed on the maintainer's machine and genuinely compiles the library crates; every binary crate stops at `linker 'link.exe' not found`, because a rustup target is a standard library and not a toolchain and the ARM64 MSVC linker was never installed. Cross-compiling was refused as the shipping path anyway: an artifact no machine of that architecture has executed is a guess with a checksum on it. **What runs on ARM64, on every commit:** `cargo fmt`, `cargo clippy`, the whole Rust test suite and the integration suites of `pl-fileio`, `pl-features`, `pl-draw`, `pl-design`, `pl-update` and `pl` — that leg is in `ci.yml`'s `test` job. **What does not:** [`tools/ci.ps1`](tools/ci.ps1), the gate this page counts the steps of, which runs on three legs and none of them is this one. So on ARM64 nothing checks the PE icon and version resources and nothing runs the differential oracles. **The archive and the installer are not in that list**: `ci.yml`'s `test` job packs the ARM64 zip, checks it from the outside, builds the MSI and installs, asserts and uninstalls it both per-user and per-machine, on every push and every pull request — those steps are guarded by `runner.os == 'Windows'` and not by architecture, so the ARM64 leg runs them too. This row, the row below it and `docs/RELEASING.md` all said the opposite until 2026-08-15 — that packaging happened only when a release was cut — which was the position x86-64 was in for six releases and had already stopped being true of ARM64 by the time it was written. **One consequence a user can actually hit, and one that was resolved by measuring it.** The resolved one: the Python extension module is *there*. `crates/pl-py` links against CPython, Windows resolves those symbols at link time, and whether the ARM64 runner carried an ARM64 CPython was an open question — an ARM64 archive is still permitted to ship without `polylinker.pyd` by declaring the omission in its own `SHA256SUMS.txt`, which [`tools/check-archive.ps1`](tools/check-archive.ps1) holds it to and prints either way, but it has not had to: the run this release was cut from logged `22 file(s), 8 licence text(s), the Python extension module present`, the same file count as the `windows-x64` zip. The one that was real, and is fixed in v0.11.1: **the ARM64 binaries of v0.11.0 did not have the static C runtime the x86-64 ones have.** `.cargo/config.toml` scoped `-C target-feature=+crt-static` to `x86_64-pc-windows-msvc` and declared nothing for `aarch64-pc-windows-msvc`, so all four ARM64 binaries imported `VCRUNTIME140.dll` — not part of Windows, redistributable needs administrator rights, the exact dependency that file exists to remove, on a machine whose owner may not be allowed to install it. Measured on the published v0.11.0 artifacts rather than inferred, with the x86-64 build of the same release clean on all four as the control. **Nothing went red, and that is the part worth keeping.** The step that asserts the absence of those imports was a step of [`tools/ci.ps1`](tools/ci.ps1), the gate runs on three legs and none is ARM64, so the one architecture whose flags were missing was the one architecture the check could not reach. v0.11.1 declares the flag for both triples and closes the gap that hid it: the scan moved to [`tools/check-crt.ps1`](tools/check-crt.ps1) so the ARM64 leg of `ci.yml` calls the same implementation on the staged payload on every push, and a new gate step reads `release.yml`'s matrix against `.cargo/config.toml` and fails any Windows platform whose triple has no `+crt-static` — a text comparison, so it runs on every leg including the ones that cannot build the artifact, and it fails before a build rather than after a release. **The `.msi` ships rather than zip-only on purpose**: half a platform reads to a user as a platform where the software does not really work, and it is also the file [`crates/pl-update/src/flow.rs`](crates/pl-update/src/flow.rs) hands over on Windows. `pl update` used to refuse outright on ARM64, and correctly — that table returns `None` for anything the release workflow does not build, so it declined rather than handing an ARM64 user an x86-64 installer; adding the entry and publishing the file are therefore one change, since an entry whose file does not exist turns a clean refusal into a 404. ARM64 Windows *emulates* x86-64, so these machines were never locked out and the `windows-x64` download has always run on them, more slowly; nobody has measured either build on one. See [`docs/RELEASING.md`](docs/RELEASING.md#windows-on-arm64), which is a table of exactly what this leg checks and what it leaves unchecked. |
| macOS disk image | **`polylinker-<version>-macos-universal.dmg`, since 2026-09-03**, beside the tarball rather than instead of it. Open it and drag `Polylinker.app` to Applications: the same three programs and the Python module inside a bundle with an icon and a name Finder shows, and every licence text under `Contents/Resources`. [`tools/build-dmg.sh`](tools/build-dmg.sh) stages the bundle from the same `SHA256SUMS.txt` the MSI is generated from, so it carries no file list of its own, and [`tools/check-dmg.sh`](tools/check-dmg.sh) mounts the result and holds the bundle to that manifest — every member with its recorded hash, nothing extra, the slices the platform label promises — on every push and on every release. On the push legs it also **launches the editor out of the mounted image** and requires it to exit cleanly; run 33763643129 logged `18 of 18 manifest member(s) present with their recorded digests`, `icon: 11 PNG entries` and `the editor opened a window from the mounted bundle and exited 0`. The icon is drawn from the same `polylinker.svg` as the Windows one, by [`bins/pl-gui/icon/build-icns.py`](bins/pl-gui/icon/build-icns.py) with nothing but the standard library, and pinned by sha256 like the `.ico`. What it does not do: it is **unsigned**, exactly like the tarball, so Gatekeeper refuses it and `xattr -dr com.apple.quarantine /Applications/Polylinker.app` is the remedy; it registers no file types, because the editor takes files from the command line and from drag-and-drop and nothing in winit or eframe delivers the event a Finder double-click on a document sends (checked in their sources rather than assumed); and `pl update` still fetches the tarball, because a disk image is neither run nor unpacked and teaching the updater a third kind is its own change. See [`docs/RELEASING.md`](docs/RELEASING.md#macos-notarisation). |
...
installation has actually exercised and, until 2026-09-03, **no CI leg anywhere
launched this program**. Since then `ci.yml`'s `gui-smoke` job opens the window
on each backend in turn and closes it from the first frame (`PL_GUI_SMOKE=1`):
on `ubuntu-latest` under Xvfb — Mesa's software OpenGL for glow, its software
Vulkan for wgpu — and on `macos-latest` on Apple's OpenGL and Metal. **All four
launches have been seen green on hosted runners**, in run 33763643129: the
Vulkan one reported `driverID = DRIVER_ID_MESA_LLVMPIPE` and `deviceName =
llvmpipe (LLVM 20.1.2, 256 bits)`, so it really was Mesa's software Vulkan and
not the GLES backend wearing wgpu's name. The Linux leg was written from the
packages' documentation and its first two runs went red, which is the argument
for having it: the editor opens its input libraries by name at run time, and
`libxkbcommon-x11-0` was on no runner. That is a Linux software adapter and one
Apple GPU, not a Windows driver, and no Windows leg launches it.
`PL_GUI_RENDERER=wgpu`
or `=glow` pins one and disables the fallback, which is how to exercise the other
on a machine where the first succeeds, and is what that job sets.
