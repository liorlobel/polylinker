...
2. A circular and linear map at or above SnapGene's visual quality, exported as clean layered SVG and PDF, byte-identically on every platform.
   ...
   >
   > **What this item still does not cover, and should not be read as covering.** The browser prototype's map is the TypeScript renderer in `packages/circular-map`, which has no track and still draws a line as a gapped ring. The desktop app's on-screen map (`bins/pl-gui/src/map.rs`) is a second implementation that paints straight to egui: since the export learned the track, the two agree about the *shape* of a linear molecule, and they still disagree about lanes, about where a feature's name goes, about how many ruler ticks there are, and about which end of the last base a feature box stops at. "Layered" is also a claim about grouping that the SVG does not yet make: it emits ordered elements with `<title>`, not named layer groups an illustrator can toggle.
...
all 89 signed off by a curator — and 110 records as of 2026-08-10, of which 89
are signed and 21 are `proposed` and therefore not searched by default; and
Type IIS
