...
5. **Talk to Bar-Ilan's technology transfer office BEFORE the first public commit.** University IP assignment policy derails more academic OSS releases than vendors do. Get written clearance for Apache-2.0.
...
**Mitigation:** §9 in full. Never accept the EULA. Never decompile. Never ask for the spec. Archive evidence now. Talk to Bar-Ilan TTO before commit #1.
...
- **Fiscal host from month one.** Open Bioinformatics Foundation (best cultural fit, already hosts Biopython), NumFOCUS, or Software Freedom Conservancy. The host holds: signing certificates, domains, the Tauri updater signing key, the GitHub org, and any grant money. This is the single structural fix for the documented cause of death.
- **Two maintainers with commit and release keys before v1.0.** Non-negotiable gate on the v1.0 release, not an aspiration.
- **DCO, not CLA.** Lower friction; sufficient for provenance. (Revisit only if the fiscal host requires otherwise.)
...
| **GitHub Sponsors / Open Collective** | Baseline | Covers the $99/yr Apple fee and a domain. Not a salary. |
...
3. **Is there a second maintainer?** Everything else is downstream of this.
...
2. **Email Bar-Ilan's technology transfer office.** Ask for written clearance to release faculty-developed software under Apache-2.0, and whether prior disclosure is required. This takes weeks; start it now.
...
5. **Check Azure Artifact Signing eligibility** for whatever entity will own this. Fifteen minutes now, or an unpleasant surprise at v1.0.
...
good as `features/features.tsv` and that is 70 records at release 2026.07.28,
