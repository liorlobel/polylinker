...
**0.5 Naming firewall.** Never use their `sseqid` scheme. `CmR_(2)`, `KanR_(3)`, `f1_ori_(3)` all exist verbatim in their file; 294 of 1,367 sseqids (21.5%) carry a `_(N)` uniquifier, and in all 167 multi-variant groups the siblings share an identical display name — that suffix pattern is a derived-list fingerprint. Our IDs are our own (`PLF:0001`…).
