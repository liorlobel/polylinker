...
Note **OP697991** in the prose-only row: it is one of the two independent submissions corroborating `PLF:4006` — a row that shipped until the curator withdrew it on 2026-08-11 — and its `/note` text is byte-identical to `MH325107`'s but for the token `label: `. That is a demonstrated false negative of the screen, and it was inside the shipping witness set when it was found. Withdrawing that one row removed that instance and nothing else: the screen is unchanged, no detector closed the gap, and widening the screen to that shape was deliberately declined because honest rows share it.
