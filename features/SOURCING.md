...
Note **OP697991** in the prose-only row: it is one of the two independent submissions corroborating `PLF:4006`, a row that ships, and its `/note` text is byte-identical to `MH325107`'s but for the token `label: `. That is a demonstrated false negative of the screen inside the shipping witness set, and widening the screen to that shape was deliberately declined because honest rows share it.
