...
**The finding.** §0.4's gate compares DESCRIPTION TEXT against `snapgene.csv`. It cannot see a COORDINATE arriving from SnapGene, and one route does exactly that. ENA folds a submitter's SnapGene `/label` into the `/note`, so an ordinary depositor who annotated their plasmid in SnapGene and deposited it publishes SnapGene's **boundary convention** inside a record this document cleared as a source (§1, NCBI/ENA, GO_WITH_CAVEAT). **15 of 481** records surveyed while building Stage 5 carry the fingerprint (3.1%; 6 by the strong `label:` tell, 9 by prose alone — the fifteen are listed below), and **90.9%** of that survey holds at least one extent that a SnapGene-annotated record also holds.
...
3. **The measured false-positive rate settles it.** Over the 55 distinct extents in the 481-record survey, the rule "fewer than two independent submissions annotate this exact extent" fires on **46 (84%)**, rising to **100%** for extents held by fewer than five submissions. **Ten of our fifteen** Class B candidates escaped only because they are the most-deposited regulatory elements in existence; **the other five did not** — that predicate is exactly `MIN_PLACEMENTS`, and the paragraph at the end of this section names the five rows it refused. The next thirty are rarer. On bGH poly(A), **57.1%** of independent non-SnapGene submissions land on SnapGene's extent by themselves — 0.8 bits of information.
4. **The ground truth is not knowable from inside the corpus.** Two independent detectors for "this record was annotated in SnapGene" — the `label:` tell and their `Feature`-column naming — agree on 481 records at **Cohen's κ ≤ 0.067**, and negatively above four names. A depositor who retypes the note by hand is invisible to every tell there is, which is precisely the case a taint check would exist for. **That is not hypothetical, and the instance is inside this database's own witness set** *(measured 2026-08-10, when `PLF:4006` was still in the table; `OP697991.1` was cited in `provenance.tsv` and `MH325107.1` named in that row's `notes` as the excluded witness. `PLF:4006` was withdrawn on 2026-08-11 and neither pointer survives it, so both accessions are written out here instead and this still re-derives from two ENA fetches and nothing else)*: `OP697991.1`, one of exactly two independent submissions corroborating `PLF:4006`'s extent, carries four `/note`s whose descriptive half is byte-identical to the corresponding `/note` in `MH325107.1` — a record the screen DOES flag — in the same two-part shape ENA emits when it folds a `/label`, differing by the token `label: ` and nothing else. The screen passes it, correctly by its own rule and wrongly as a matter of fact. Widening the rule to that shape is not obviously right either: `MN224159.1` and `OR659033.1`, which corroborate `PLF:4010`, use the same short names with no descriptive half at all, and those short names are simply what the elements are called.


**The fifteen fingerprinted records, so the figure is re-derivable** *(added 2026-08-10)*. The survey figure read "14 of 481 (2.9%)" in three places until it was re-measured over the preserved corpus, which gives **15 of 481 (3.1%)**. It is split here because the two tells are not the same claim: `label:` is SnapGene's own token surviving ENA's fold of `/label` into `/note`, while a prose match is only a description this project's detector recognises, which a depositor could have written independently.

| tell | n | accessions |
|---|---|---|
| strong (`label:` present) | 6 | MH325107, MH325111, MN811116, MN811118, MW879730, MW879738 |
| prose only | 9 | JX006096, MK681507, OP697991, PP539712, PP539713, PP539716, PP539717, PP539718, PP539719 |

Listing them is the point. This section previously recorded that the record list "was not preserved, and nothing in this repository rebuilds it", which made the headline figure of §0.6 exactly the kind of number a reader has to take on faith — the thing this document exists to refuse. Fetch these fifteen from ENA and the count is checkable; the other 466 are not needed to check it, only to reproduce the denominator.

Note **OP697991** in the prose-only row: it is one of the two independent submissions corroborating `PLF:4006` — a row that was in the table until the curator withdrew it on 2026-08-11 — and its `/note` text is byte-identical to `MH325107`'s but for the token `label: `. That is a demonstrated false negative of the screen, and it was inside the table's witness set when it was found. (`PLF:4006` never shipped in the sense `Db::reviewed()` means: no Class B row is signed, and none of them has ever been searched by `pl annotate`.) Withdrawing that one row removed that instance and nothing else: the screen is unchanged, no detector closed the gap, and widening the screen to that shape was deliberately declined because honest rows share it.

**Which of those four a reader can re-run, and which they cannot** *(added 2026-08-10, on review)*. **Point 1 re-derives from the tree, and was re-measured on that date**: fetch the pin `taint_gate.py` names and `snapgene.csv` has exactly the four columns `sseqid, Feature, Type, Description` over 1,367 rows, holds no value anywhere that is a coordinate, a span, or a nucleotide string, and has exactly one row (0.1%) stating a length in prose — so the claim that there is nothing in the artifact to compare an extent *against*, which is the load-bearing one, is checkable and holds. So do §0.5's figures: 294 of 1,367 sseqids (21.5%) carry `_(N)`, and all 167 multi-variant groups have siblings sharing one display name. **Points 3 and 4 do not re-derive.** Their figures — 90.9%, 46 of 55, κ ≤ 0.067, 57.1% — come from a one-off corpus of 481 INSDC records assembled while building Stage 5; that record list was not preserved, and nothing in this repository rebuilds it. They are cited as the evidence for a decision **not to build** something, which is the one use that survives being unreproducible. Anyone reaching for them to justify anything else has to rebuild the survey and commit it first.
...
*Status 2026-08-10 — narrowed, not closed, and the distinction is the point.* Of the five named markers, three ship signed and did so before this date: HygR (`PLF:0019`), ZeoR (`PLF:0018`) and NeoR (`PLF:0003`). Stage 2 added the remaining two, `pac` (`PLF:1014`) and `bsd` (`PLF:1015`), on 2026-08-10 — but both are `proposed`, so no curator has read them and `pl annotate` does not search them; a gap is not closed by a row nobody has signed. **The codon-optimised half is untouched and is not on a path to being closed by this route at all:** all fourteen Stage 2 rows are native CDS matched residue-for-residue to a UniProt canonical, and `stage_uniprot.py` says in `pac`'s own caveat that a codon-optimised `pac` "cannot match at all" at the nucleotide tier. §3 puts that case on the translated tier, which is a matcher and not a source. So what remains of this gap is: two rows awaiting a signature, and every codon-optimised form.
